...
const ballW = basePos.width;
const ballH = basePos.height;

const radiusY = (basePos.height / 2) * 1.25;
const radiusX = (basePos.width / 2) * 1.25;
const ballSize = 10;
...
window.addEventListener("mousemove", (e) => {
    ...
    const radius = Math.abs(yPos) <= ballH ? radiusX : radiusY;
    ...
    requestAnimationFrame(() => {
        if (diagonal > radius) {
            ...
            animateElTo(ball, { x: ballX, y: ballY });
...
function animateElTo(el, options) {
    console.log("animate el to", { options });
    console.log({ transform: el.style.transform });
    const targetX = options.x;
    const targetY = options.y;
    const [a,b,c,d,x,y] = window.getComputedStyle(el).getPropertyValue("transform").match(/(-?[0-9\.]+)/g).map(Number);

    if(targetX == x && targetY == y){
        return;
    }

    const animateX = lerp(x, targetX, 0.1);
    const animateY = lerp(x, targetX, 0.1);

    console.log({ x,y });

    requestAnimationFrame( () => {
        ball.style.transform = `translate(${animateX}px, ${animateY}px)`;
        animateElTo(el,options);
    } );
